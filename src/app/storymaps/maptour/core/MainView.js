define(["storymaps/maptour/core/WebApplicationData",
		...
		"storymaps/maptour/ui/desktop/CentricLayout",
		// Mobile UI
		...
		domGeom)
	{
		return function MainView() 
		{
			...
			{			
				...
				*/
				// TODO NEED SOLUTION FOR PRODUCTION
				/*var centricLayout = true;
				if(centricLayout && has("ie") && has("ie") < 9) {
					_core.initError("oldBrowserExplain2");
					return false;
				}*/
				...
				// New layout UI
				//TODO: too early to tell if new layout, but dont initiate if not new layout
				app.centricLayout = new CentricLayout(app.isInBuilderMode);
				...
				topic.subscribe("CORE_UPDATE_CENTER_MAP", this.centerMap);
				...
				$(document).bind(
					'touchmove',
					function(e) {
						if( ! $(e.target).parents('#helpPopup, #placardContainer, #introPanel, #infoPanel, #popupViewGeoTag, .introTextArea, .portraitTextArea, .landscapeTextArea, .btn-fullscreen').length && ! $(e.target).hasClass('subtitle') )
							e.preventDefault();
					}
				);
			...
			this.webmapLoaded = function()
			{
				...
				app.desktopPicturePanel.init(WebApplicationData.getColors()[1], !app.data.sourceIsNotFSAttachments(), MapTourHelper.isModernLayout());
			...
			function initUI()
			{
				...
				topic.subscribe("maptour-start-tour", loadNextPicture);
			...
			this.resize = function(cfg)
			{
				...
				if( app.mapTips ) {
					if ( cfg.isMobileView )
						...
						app.mapTips.show();
					else if ( app.data.getCurrentGraphic() )
						if(app.data.getCurrentGraphic().attributes.getName() !== "")
							app.mapTips.show();
					else
						return;
				}
			...
			function selectedPointChange_after(forcedRecord)
			{
				...
				if ($('body').hasClass('centric-layout')) {
					app.centricLayout.updatePoint(app.data.getCurrentIndex(), attributes);
				}
			}
			...
			function selectedPointChange_afterStep2()
			{
				...
				if( app.isFirstUserAction || $('body').hasClass('centric-layout')) {
			...
			{
				var index = app.data.getCurrentIndex();
				if ( ! index && (!app.data.hasIntroRecord() && $('body').hasClass('centric-layout') || !$('body').hasClass('centric-layout')))
					return;
				
				if(app.centricLayout.introShowing())
					return;
			...
			function loadPictureAtIndex(index, dotClick)
			{
				// Intro record
				...
				if (index != app.data.getCurrentIndex() || (index === app.data.getCurrentIndex() && dotClick && app.data.hasIntroRecord())) {
					selectedPointChange_before();
					app.data.setCurrentPointByIndex(index);
					if (app.data.hasIntroRecord() && dotClick && index === 0) {
						app.data.setCurrentPointByIndex(null);
						setTimeout(function(){
							selectedPointChange_after(app.data.getIntroData());
						}, 0);
					}
					else if (app.data.hasIntroRecord() && dotClick) {
						app.data.setCurrentPointByIndex(index-1);
						setTimeout(function(){
							selectedPointChange_after(null, dotClick);
						}, 0);
					}
					else {
						selectedPointChange_before();
			...
			function moveGraphicToFrontStep2(graphic)
			{
				...
				if( app.isInBuilderMode ){
					...
					app.mapTips = new MultiTips({
						...
						mapAuthorizedMinWidth: $('body').hasClass('centric-layout') ? $('#smallImageContainer').width() + 60 : -1,
			...
			{
				var isDesktopModernUI = (MapTourHelper.isModernLayout() || $('body').hasClass('centric-layout')) /*&& ! $('#mainMap').hasClass('circleMap')*/ && ! MapTourHelper.isOnMobileView();
				...
				if($('body').hasClass('centric-layout')){
					visibleExtent = new Extent(
						app.map.extent.xmin + ((domGeom.position(dom.byId("smallImageContainer")).x + $('#smallImageContainer').width()) * app.map.getResolution()),
			...
			{
				var isDesktopModernUI = (MapTourHelper.isModernLayout() || $('body').hasClass('centric-layout')) && ! MapTourHelper.isOnMobileView() && ! $('#mainMap').hasClass('circleMap');
				if( zoomLevel === undefined && (! isDesktopModernUI || ! app.map.__LOD) )			
					...
					if ($('body').hasClass('centric-layout')) {
						if ($('#mainMap').hasClass('circleMap')) 
							return;
						offsetX = -((20 + domGeom.position(dom.byId("smallImageContainer")).x + $('#smallImageContainer').width()) / 2);
					}
					...
					if (geom.spatialReference.wkid == 4326 && app.map.spatialReference.wkid == 102100) 
						...
						if (geom.spatialReference.wkid == 102100 && app.map.spatialReference.wkid == 4326) 
							center = webMercatorUtils.webMercatorToGeographic(geom);
						// At least don't crash the app
						else 
							if (geom.spatialReference.wkid != app.map.spatialReference.wkid) 
								return;
					if (!zoomLevel) 
						app.map.centerAt(center.offset(offsetX * app.map.getResolution(), -offsetY * app.map.getResolution()));
					else 
						app.map.centerAndZoom(center.offset(offsetX * app.map._params.lods[zoomLevel].resolution, -offsetY * app.map._params.lods[zoomLevel].resolution), zoomLevel);
				}
			};
			...
			{
				var isDesktopModernUI = (MapTourHelper.isModernLayout() && ! MapTourHelper.isOnMobileView()) || ($('body').hasClass('centric-layout') && ! MapTourHelper.isOnMobileView() && ! $('#mainMap').hasClass('circleMap'));			
				...
				if( ! isDesktopModernUI )
					...
					//TODO adjust extent for map centric view
					if ($('body').hasClass('centric-layout')) {
						if($('#mainMap').hasClass('circleMap'))
							return;
						offsetX = domGeom.position(dom.byId("smallImageContainer")).w;
						offsetY = $('.dotsContainer').height();
						extentResX = extent.getWidth() / app.map.width;
						extentResY = extent.getHeight() / app.map.height;
						
						newExtent = new Extent({
							xmin: extent.xmin + extentResX * offsetX,
						...
						offsetX = 10 + domGeom.position(dom.byId("picturePanel")).w;
						offsetY = 10 + domGeom.position(dom.byId("footer")).h;
						extentResX = extent.getWidth() / app.map.width;
						extentResY = extent.getHeight() / app.map.height;
					...
					*/
					...
					if( level != -1 ) {
						...
						if ($('body').hasClass('centric-layout')) {
							...
							var visibileContainerWidth = $('#centricIntroPanel').css('display') === 'block' ? $('#centricIntroPanel').width() : domGeom.position(dom.byId("smallImageContainer")).w;
							var containerScreenRatio = $('#centricIntroPanel').css('display') === 'block' ? 4 : 2;
							newCenter = new Point(
								extent.getCenter().x + (app.map.width / containerScreenRatio - (visibileContainerWidth * 1.5)) * lods[level].resolution,
								extent.getCenter().y - (10 + $('.dotsContainer').height() / 2) * lods[level].resolution,
