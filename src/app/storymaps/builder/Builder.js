define(["esri/arcgis/Portal",
		...
		"storymaps/builder/BuilderPanel2",
		...
		topic)
	{
		...
		var _builderPanel2 = new BuilderPanel2(
				$('.builderPanel'),
				saveAppThenWebmap,
				builderDirectCreationFirstSave,
				builderGalleryCreationFirstSave
		...
		function init(core, builderView)
		{
			...
			$(document).ready(lang.hitch(this, function(){
				...
				_builderPanel2.init(_builderView);
				...
				topic.subscribe("BUILDER_INCREMENT_COUNTER", _builderPanel2.incrementSaveCounter);	
		...
		function appInitComplete()
		{
			...
			_builderPanel2.updateSharingStatus();
			...
			if( param.src == "title" ) {
				if( param.value != app.data.getWebMapItem().item.title ) {
					if( param.value != WebApplicationData.getTitle() ) {
						...
						_builderPanel2.incrementSaveCounter();
					}
				}
				else
		...
		function appSaveFailed(source, error)
		{
			...
			_builderPanel2.saveFailed(source, error);
		}
