define(["esri/map",
		...
		Extent)
	{
		...
		function webMapInitCallback(response)
		{
			...
			//TODO THIS ENSURES THE CONFIG LAYOUT OPTIONS IS RESPECTED FOR NEW LAYOUT
			applyUILayout(/*Helper.getUrlParams().layout || WebApplicationData.getLayout() ||*/ configOptions.layout);
			...
			var showEditButton = ! app.isInBuilderMode && ( (! isProd() && Helper.getAppID(isProd())) || isProd() && app.data.userIsAppOwner() );
			var linkText = WebApplicationData.getHeaderLinkText() === undefined ? APPCFG.HEADER_LINK_TEXT : WebApplicationData.getHeaderLinkText();
			var linkURL = WebApplicationData.getHeaderLinkURL() === undefined ? APPCFG.HEADER_LINK_URL : WebApplicationData.getHeaderLinkURL();
			var socialCfg = WebApplicationData.getSocial();
		...
		function appInitComplete()
		{
			...
			// Map command buttons
			app.mapCommand = new MapCommand(app.map, function(){
				_mainView.setMapExtent(Helper.getWebMapExtentFromItem(app.data.getWebMapItem().item));
			}, _mainView.zoomToDeviceLocation, APPCFG.DISPLAY_LOCATE_BUTTON || WebApplicationData.getZoomLocationButton());
		...
		{
			$("body").removeClass("modern-layout centric-layout");

			// TODO
			//layout = "centric-map";
			
			if(layout === 'centric-media' || 'centric-map'){
				// TODO NEED SOLUTION FOR PRODUCTION
				if(has("ie") && has("ie") < 9) {
					initError("oldBrowserExplain2");
					return false;
				}
				var view = layout === 'centric-media' ? "#largeImageContainer" : "#smallImageContainer";
				app.centricLayout.currentView = view;
			}
			if (layout == "centric-map"){
				$('#mainMap').width('100%');
				$('#mainMap').height('100%');
			}
			if (layout == "centric-media" || layout == "centric-map")
				$("body").addClass('centric-layout');
			else if (layout == "integrated")
				$("body").addClass("modern-layout");
				
			if(!$("body").hasClass('centric-layout'))
				$('#mainMap').addClass('fullWidth');
		}
		...
		function updateUI(params)
		{
			...
			applyUILayout(configOptions.layout);
		...
		function handleWindowResize()
		{
			...
			var heightHeader = $("#header:visible").height() + $(".builderPanel:visible").height();
